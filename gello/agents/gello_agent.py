...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    ...
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTA2U2CD-if00-port0": DynamixelRobotConfig(
        ...
        joint_offsets=(
            3*np.pi/2, 2*np.pi/2, 3*np.pi/2, 3*np.pi/2, 3*np.pi/2, 1*np.pi/2, -1*np.pi/4
    ...
    # here for frank's laptop
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTA2U2FX-if00-port0": DynamixelRobotConfig(
        joint_ids=(0,1,2,3,4,5,6),
        joint_offsets=(
<<<<<<< HEAD
            3*np.pi/2, 2*np.pi/2, 3*np.pi/2, 3*np.pi/2, 3*np.pi/2, 1*np.pi/2, 1*np.pi/2
        #   0*np.pi/2, 0*np.pi/2, 0*np.pi/2, -1*np.pi/2, 0*np.pi/2, 1*np.pi/2, -1*np.pi/2
            # 0*np.pi/2, 2*np.pi/2, 4*np.pi/2, 3*np.pi/2, 1*np.pi/2, 2*np.pi/2, 4*np.pi/2 
        ),
        joint_signs=(1, 1, 1, 1 ,1, 1 ,-1),
        gripper_config=(7, 18.5, -23.3),
=======
            3*np.pi/2, 2*np.pi/2, 4*np.pi/2, 3*np.pi/2, 4*np.pi/2, 3*np.pi/2, 0*np.pi/2
            # 0*np.pi/2, 0*np.pi/2, 0*np.pi/2, -1*np.pi/2, 0*np.pi/2, 1*np.pi/2, -1*np.pi/2
            # 0*np.pi/2, 2*np.pi/2, 4*np.pi/2, 3*np.pi/2, 1*np.pi/2, 2*np.pi/2, 4*np.pi/2 
        ),
        joint_signs=(1, -1, 1, 1 ,1, -1 ,1),
        gripper_config=(7, 201.508984375, 159.708984375),
>>>>>>> 4f09f21a636676a8c184ad258aa62748198d77ab
    )
}

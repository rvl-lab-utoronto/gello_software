...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    ...
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTA2U2CD-if00-port0": DynamixelRobotConfig(
        ...
        joint_offsets=(
            1*np.pi/2, 2*np.pi/2, 1*np.pi/2, 3*np.pi/2, 3*np.pi/2, 1*np.pi/2, 7*np.pi/4
            # 1*np.pi/2, 2*np.pi/2, 1*np.pi/2, 3*np.pi/2, 3*np.pi/2, 1*np.pi/2, 4*np.pi/2
        ),
        joint_signs=(1, -1, 1, 1 ,1, 1 ,1),
        gripper_config=(7, 18.696484375, -23.103515625),
    )
}

...
class ROSCamera(CameraDriver):
    def __init__(self, depth_image_topic_name: str = "/depth/image_raw", rgb_image_topic_name: str = "/rgb/image_raw", init_node: bool = True):
        if(init_node):
            try:
                rospy.init_node("image_subscriber", anonymous=True)
            except rospy.exceptions.ROSException as e:
                print("Node has already been initialized, not doing a new one for image susbscriber")        
        self.rgb_topic_name = rgb_image_topic_name
        self.depth_topic_name = depth_image_topic_name
        self.rgb_image_msg = None
        self.depth_image_msg = None
        self.cv_bridge = CvBridge()
        rospy.Subscriber(rgb_image_topic_name, Image, self.rgb_image_callback)
        rospy.Subscriber(depth_image_topic_name, Image, self.depth_image_callback)

        while(self.rgb_image_msg is None or self.depth_image_msg is None):
            print(f"Waiting for ROS image msg in topics {rgb_image_topic_name, depth_image_topic_name}")
            time.sleep(1)
            if (rospy.is_shutdown()):
                return
        print(f"ROS Camera node initialized without issues")

    def rgb_image_callback(self, rgb_img_msg):       
        self.rgb_image_msg = rgb_img_msg

    def depth_image_callback(self, depth_img_msg):       
        self.depth_image_msg = depth_img_msg

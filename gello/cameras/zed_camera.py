#This file is based on https://github.com/droid-dataset/droid/blob/main/droid/camera_utils/camera_readers/zed_camera.py
...
def gather_zed_camera_by_id(id: int):
    try:
        cameras = sl.Camera.get_device_list()
    except NameError:
        print("No ZED cameras found!!!!")
        exit()
    for cam in cameras:
        if cam.serial_number == id:
            return ZEDCamera(cam)
    raise Exception(f"Sorry, Zed camera with id : {id} not found")

def list_zed_camera_ids():
    try:
        cameras = sl.Camera.get_device_list()
    except NameError:
        print("No ZED cameras found!!!!")
        return []  
    for cam in cameras:
        print(f"Zed camera with ID {cam.serial_number} is found")
...
class ZEDCamera(CameraDriver):
    ...
    def read(
        ...
        data_dict, timestamp_dict = self.read_camera() 
        left_image = data_dict["image"][self.serial_number + "_left"]
        right_image = data_dict["image"][self.serial_number + "_right"]
        rgb_images = np.array([left_image, right_image])
        depth_image = np.array([])
        return (rgb_images, depth_image)
